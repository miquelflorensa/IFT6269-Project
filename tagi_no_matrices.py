...
def InitializeParameters(units):
    ...
    params = {
        ...
        "mb": [],
        "Sb": [],
    }  # mean and variance for weights and biases
    ...
    for i in range(0, len(units) - 1):
        ...
        for j in range(no):
            ...
            for k in range(ni):
                ...
                Sa = states["Sa"][i][k]
                sum_Sz += (
                    (params["mw"][i][k][j] * params["mw"][i][k][j] * Sa)
                    + params["Sw"][i][k][j] * Sa
        ...
        for j in range(ni):
            for k in range(no):
                ...
                cov["cov_z_z"][i][j][k] = (
                    states["Sz"][i][j] * states["J"][i][j] * params["mw"][i][j][k]
                )
...
def states_feed_backward(y, params, units, states, cov, sigma_v):
    ...
    deltas["dmz"][-1][0] = states["mz"][-1][0] + (states["cov_yz"][-1][0] / Sy) * (
        y - my
    )
    deltas["dSz"][-1][0] = (
        states["Sz"][-1][0] - (states["cov_yz"][-1][0] / Sy) * states["cov_yz"][-1][0]
    )
    ...
    for i in range(len(units) - 2, -1, -1):
        ...
        for j in range(ni):
            for k in range(no):
                ...
                deltas["dmw"][i][j][k] = params["mw"][i][j][k] + Jw * (
                    deltas["dmz"][i + 1][k] - states["mz"][i + 1][k]
                )
                deltas["dSw"][i][j][k] = (
                    params["Sw"][i][j][k]
                    + Jw * (deltas["dSz"][i + 1][k] - states["Sz"][i + 1][k]) * Jw
                )
        for j in range(no):
            Jb = cov["cov_z_b"][i][j] / states["Sz"][i + 1][k]
            deltas["dmb"][i][j] = params["mb"][i][j] + Jb * (
                deltas["dmz"][i + 1][j] - states["mz"][i + 1][j]
            )
            deltas["dSb"][i][j] = (
                params["Sb"][i][j]
                + Jb * (deltas["dSz"][i + 1][j] - states["Sz"][i + 1][j]) * Jb
            )
        ...
        if i > 0:
            ...
            for j in range(ni):
                ...
                for k in range(no):
                    ...
                    aux_Sz += (
                        Jz * (deltas["dSz"][i + 1][k] - states["Sz"][i + 1][k]) * Jz
                    )
...
def main():
    ...
    for i in range(epochs):
        ...
        for j in range(iterations):
            ...
            # Feed backward
            params, states, cov = states_feed_backward(
                y_batch, params, units, states, cov, sigma_v
            )
    ...
    # Plot data
    plt.plot(x_test_sorted, y_test_sorted, "o", color="black", alpha=0.8)
    plt.plot(x_test_sorted, y_pred_sorted, "o", color="red")

    # Plot variances
    plt.fill_between(
        x_test_sorted,
        y_pred_sorted - sy_pred_sorted.flatten(),
        y_pred_sorted + sy_pred_sorted.flatten(),
        color="red",
        alpha=0.3,
    )
    ...
    plt.ylabel("$y$")
    plt.legend(
        [r"$y_{true}$", r"$\mathbb{E}[Y']$", r"$\mathbb{{E}}[Y'] \pm {} \sigma$"],
        loc="upper left",
    )

    # plt.show()
    plt.tight_layout()
    plt.savefig("toy_example.pdf", transparent=True)
